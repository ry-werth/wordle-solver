...
word_list_no_repeats = [x for x in word_list if len(set(x)) == len(x)]
...
with open('word_placement_dict.pkl', 'rb') as f:
    word_placement_dict = pickle.load(f)

def word_score(word):
    placement_count_list = word_placement_dict[word]
    word_score = sum(placement_count_list)
